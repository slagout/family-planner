...
export function Navbar() {
  ...
  return (
    <nav className="bg-white dark:bg-neutral-900 shadow-sm border-b border-neutral-200 dark:border-neutral-800">
      ...
      {/* Mobile menu */}
      {open && user && (
        <div className="sm:hidden border-t border-neutral-100 dark:border-neutral-800 px-4 py-3 flex flex-col gap-1 bg-white dark:bg-neutral-900">
          {navLink('/', 'Planner')}
          {navLink('/recipes', 'Recipes')}
          {navLink('/pantry', 'Pantry')}
          {navLink('/shopping', 'Shopping')}
          <button
            onClick={handleLogout}
            className="mt-2 text-left px-3 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-950 rounded-md"
          >
            Logout
          </button>
        </div>
