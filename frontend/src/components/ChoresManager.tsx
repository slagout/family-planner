...
export function ChoresManager() {
  ...
  const handleAddChore = async (e: React.FormEvent) => {
    ...
    await submitChore(formData, token);
  };

  const submitChore = async (data: typeof formData, token: string) => {
    ...
    try {
      ...
      setFormData({
        title: '', description: '', frequency: 'weekly', rewardPoints: '10',
        childId: '', dueDate: '', assignedToGroup: false, groupChildIds: [], splitType: 'equal',
      });
      setShowForm(false);
    } catch (err: any) {
  ...
  const handlePinUnlocked = async (token: string) => {
    setShowPin(false);
    if (pendingSubmit) {
      await submitChore(pendingSubmit, token);
  ...
  const frequencyLabel: Record<string, string> = {
    daily: 'ðŸ“… Daily', weekly: 'ðŸ“… Weekly', monthly: 'ðŸ“… Monthly', once: 'ðŸŽ¯ Once',
  };
  ...
  return (
    <div className="bg-white dark:bg-gray-200 rounded-xl shadow-sm border border-gray-100 dark:border-gray-300 p-6">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-900">âœ“ Chores</h2>
        <button
          onClick={() => setShowForm(!showForm)}
          className="px-4 py-2 bg-primary-500 hover:bg-primary-600 text-white rounded-lg text-sm font-medium transition-colors"
        >
          {showForm ? 'âœ• Cancel' : 'âž• Add Chore'}
        </button>
      </div>
      ...
      {showForm && (
        ...
        >
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              <label className="relative inline-flex items-center cursor-pointer">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, assignedToGroup: e.target.checked, childId: '', groupChildIds: [] })
                  }
                  className="sr-only peer"
                />
                <div className="w-11 h-6 bg-gray-200 dark:bg-gray-300 peer-focus:ring-2 peer-focus:ring-primary-400 rounded-full peer peer-checked:bg-primary-500 transition-colors" />
              ...
              {formData.assignedToGroup && (
                <span className="text-xs text-gray-500 dark:text-gray-600 ml-1">
                  â„¹ï¸ Points split equally on collaborative completion
                </span>
              )}
            </div>
            ...
            {!formData.assignedToGroup ? (
              <div>
                <label className={labelClass}>Assign to Child</label>
                <select
              ...
              <div>
                <label className={labelClass}>Select Children (min 2)</label>
                  ...
                  {children.map((child) => (
                    <label key={child.id} className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        className="w-4 h-4 text-primary-500 rounded border-gray-300 dark:border-gray-400"
                      />
                      <span className="text-sm text-gray-700 dark:text-gray-800">{child.name}</span>
                    </label>
                  ))}
                </div>
                <p className="text-xs text-blue-600 mt-2">
                  ðŸ’¡ Points will be split equally when all assigned children complete this task.
                </p>
              </div>
            )}
            ...
            >
              {isSubmitting ? 'Creatingâ€¦' : 'Add Chore'}
            </button>
          </div>
        </form>
      )}
      ...
      <div className="mb-4">
        <label className={labelClass}>Filter by Child</label>
        <select
          value={filterChildId}
          onChange={(e) => setFilterChildId(e.target.value)}
          className="px-3 py-2 border border-gray-300 dark:border-gray-400 rounded-lg bg-white dark:bg-white text-gray-900 dark:text-gray-900 focus:outline-none focus:ring-2 focus:ring-primary-500"
        >
      ...
      {loading ? (
        <div className="text-center py-8 text-gray-500 dark:text-gray-600">Loading choresâ€¦</div>
      ) : chores.length === 0 ? (
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <h3 className="font-semibold text-gray-900 dark:text-gray-900">{chore.title}</h3>
                  {chore.description && (
                    <p className="text-sm text-gray-600 dark:text-gray-700 mt-1">{chore.description}</p>
                  )}
                  <div className="flex flex-wrap gap-2 mt-2">
                    ...
                    <span className="text-xs bg-purple-50 text-purple-700 px-2 py-1 rounded-full">
                      â­ {chore.rewardPoints} pts
                    </span>
                    {chore.assignedToGroup && (
                      <span className="text-xs bg-indigo-50 text-indigo-700 px-2 py-1 rounded-full">
                        ðŸ‘¥ Group ({chore.splitType} split)
                      </span>
                    )}
                  </div>
                </div>
